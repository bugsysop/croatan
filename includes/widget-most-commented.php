 ...
 * @since 0.1.0
 */
class Croatan_Widget_Most_Commented extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			...
			'description' => __( 'Display top commented posts.', 'croatan' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget( 'most-commented', __( 'Most Commented', 'croatan' ), $widget_options, $control_options );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( $loop->have_posts() ) {
			...
			while ( $loop->have_posts() ) {
				...
				comments_number( __( '(0)', 'croatan' ), __( '(1)', 'croatan' ), __( '(%)', 'croatan' ), 'comments-link', '' );
				echo '<span>';
				echo '</li>';
			}
	...
	function form( $instance ) {
		...
		/* Set up the defaults. */
		$defaults = array(
			'title' => __( 'Most Commented', 'croatan' ),
			'post_type' => 'post',
			'posts_per_page' => 10,
			'year' => '',
			'monthnum' => '',
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_type' ); ?>"><?php _e( 'Post Type:', 'croatan' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'posts_per_page' ); ?>"><?php _e( 'Limit:', 'croatan' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'year' ); ?>"><?php _e( 'Year:', 'croatan' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'monthnum' ); ?>"><?php _e( 'Month:', 'croatan' ); ?></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'day' ); ?>"><?php _e( 'Day:', 'croatan' ); ?></label> 
